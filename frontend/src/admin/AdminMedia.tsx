      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {items.map((item) => (
          <div key={item._id} className="border border-slate-100 rounded-xl p-3 bg-white">
            <img src={`${API_BASE}${item.url}`} alt={item.filename} className="w-full h-32 object-cover rounded-lg" />
            <p className="text-xs text-slate-600 mt-2 truncate">{item.filename}</p>
            <div className="flex items-center gap-3 mt-2">
              <button onClick={() => copyUrl(`${API_BASE}${item.url}`)} className="text-xs text-blue-600">Copy URL</button>
